...
# ─── build-time args (NO GitHub token!) ──────────────────────────────────────
...
# ---------- fetch model (.h5) & convert to TF-JS -----------------------------
RUN set -eux; \
    if [ -n "${HF_MODEL_URL}" ]; then \
        ...
        asset_url="$(echo "${info}" | jq -r --arg n "${MODEL_NAME}" \
            'if type=="array" then                     # /releases  → array
                 .[]?.assets[]? | select(.name==$n) | .browser_download_url
             else                                      # /releases/latest or /tags/X → object
                 .assets[]?    | select(.name==$n) | .browser_download_url
             end' )"; \
        [ -n "${asset_url}" ] && [ "${asset_url}" != "null" ] \
            || { echo "ERROR: ${MODEL_NAME} not found"; exit 1; }; \
...
# ---------- shim: re-enable CUDA if runner actually has a GPU ---------------
RUN printf '%s\n' \
'#!/usr/bin/env bash' \
'set -e' \
