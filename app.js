...
/* remove anything after “Type…”, percentages, or common Smogon verbs */
const cleanMove = m=>{
...
/* ---------- NEW line cleaners ---------- */
const cleanItem = txt => txt.replace(/\bHolder'?s.*$/i,"").trim();          /* strip Holder’s clause   */
const cleanAbility = txt => txt.replace(/This Pokémon.*/i,"").trim();       /* strip explanatory text  */
const cleanLine = txt => txt.replace(/^[-–•\s]+/,"").trim();                /* remove leading bullets */
...
function renderTierSets(list,container){
  ...
  const frag=document.createDocumentFragment();   /* NEW: batch DOM */
  ...
  list.forEach((src,i)=>{
    ...
    const pushLine=lineRaw=>{
      ...
      if(/this pokemon'?s /i.test(low)){
        set.ability=cleanAbility(line); return;
      }

      /* items */
      if(low.startsWith("item:")||
         /(choice|band|scarf|boots|orb|helmet|leftovers|berry|vest|plate|seed|belt)/i.test(low)){
    ...
    set.moves=[...new Set(movesClean.map(cleanMove))];

    if(credits.length) set.credits=[...new Set(credits)].join(" • ");

    /* remove accidental double separators like " /  /" */
    ...
    hdr.onclick=()=>{
      ...
      hdr.querySelector(".chev").style.transform=card.classList.contains("collapsed")?"rotate(0deg)":"rotate(90deg)"; /* NEW */
    };
    hdr.querySelector(".chev").textContent=card.classList.contains("collapsed")?"▸":"▾";
...
document.addEventListener("keydown",e=>{
  ...
  if(e.key==="ArrowLeft"||e.key==="[") toggleAllSets(false);   /* NEW */
  if(e.key==="ArrowRight"||e.key==="]") toggleAllSets(true);   /* NEW */
});
