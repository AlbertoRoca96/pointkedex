...
def load_labels() -> Dict[int, str]:
    ...
    if all(k.isdigit() for k in raw):     # {"0":"Bulbasaur"}
        return {int(k): v for k, v in raw.items()}
    if all(isinstance(v, int) for v in raw.values()):  # {"Bulbasaur":0}
...
print(f"[✓] {len(IDX2NAME)} labels, {len(POKEDEX)} dex entries", file=sys.stderr)
...
def predict() -> Any:
    ...
    try:
        ...
        conf = float(prob.max()); idx = int(prob.argmax()); name = IDX2NAME.get(idx,"Unknown")
    except Exception as e:
        return jsonify({"error":str(e)}), 500

    dq = _recent.setdefault(cid(), deque(maxlen=STABLE_CNT)); dq.append((idx, conf))
...
@app.route('/pointkedex/api/pokemon/<slug>')          # for GH-Pages
